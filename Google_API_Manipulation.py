...
def get_credentials():
    ...
    if not credentials or credentials.invalid:
        ...
        log_file().info("Source Compiler.log", "Storing credentials to " + credential_path)
...
def create_folder(name):
    ...
    file = drive_service.files().create(body=file_metadata,
                                        fields='id').execute()
    log_file().info("Source Compiler.log", "Created folder \"" + name + "\" (ID: %s)" % file.get('id'))
...
def clean_folder(folder_id):
    ...
    if (delete == False):
        log_file().info("Source Compiler.log", "No files found to remove from " + folder_name + " (ID: " + parent_id + ").")
        print("No files found to remove from " + folder_name + " (ID: " + parent_id + ").")
    else:
        log_file().info("Source Compiler.log", "Finished cleaning folder from " + folder_name + " (ID: " + parent_id + ").")  
...
def rename_file(file_id, newname):
    ...
    try:
        ...
        log_file().info("Source Compiler.log", "File \"{0}\" renamed as: {1} (ID: {2}).".format(filename, newname, file_id))
        print("File \"{0}\" renamed as: {1} (ID: {2}).".format(filename, newname, file_id))
    except TypeError:
        log_file().error("Source Compiler.log", "Could not rename file.")
...
def dl_file(file_id, file_name):
    ...
    log_file().info("Source Compiler.log", "Downloading " + file_name + " (ID: " + file_id + ").")
    print("Downloading " + file_name + " (ID: " + file_id + ").")
    while done is False:
        status, done = downloader.next_chunk()
        log_file().info("Source Compiler.log", "Download %d%%." % int(status.progress() * 100))
...
def dl_folder(folder_id):
    ...
    while True:
        ...
        for file in response.get('files', []):
            ...
            if (extension == 'xls') or (extension == 'xlsx') or (extension == 'csv') or (extension == 'pdf'):
                files_exist = True
                log_file().info("Source Compiler.log", "Found file: %s (ID: %s)" % (file_name, file_id))
    ...
    if (not items or files_exist == False):
        log_file().info("Source Compiler.log", "No files found to download from " + folder_name + " (ID: " + parent_id + ").")
        print("No files found to download from " + folder_name + " (ID: " + parent_id + ").")
    else:
        log_file().info("Source Compiler.log", "Finished downloading files from " + folder_name + " (ID: " + parent_id + ").")
...
def get_filenames_in_folder(folder_id):
    ...
    while True:
        ...
        for file in response.get('files', []):
            ...
            if (extension == 'xls') or (extension == 'xlsx') or (extension == 'csv') or (extension == 'pdf'):
                files_exist = True
                log_file().info("Source Compiler.log", "Found file: %s (ID: %s)" % (file_name_full, file_id))
    ...
    items = response.get('files', [])
    if (not items or files_exist == False):
        log_file().info("Source Compiler.log", "No files found in " + folder_name + " (" + parent_id + ").")
        print("No files found in " + folder_name + " (" + parent_id + ").")
    else:
        log_file().info("Source Compiler.log", "Finished retrieving file names from " + folder_name + " (" + parent_id + ").")
...
def find_file_id(filename):
    ...
    if file_id == None:
        log_file().warning("Source Compiler.log", "File \"%s\" not found." % filename)
...
def find_file_name(file_id):
    ...
    if file_name == None:
        if file_id == None:
            log_file().warning("Source Compiler.log", "No file ID given.")
            print("No file ID given.")
        else:
            log_file().warning("Source Compiler.log", "File with ID \"" + file_id + "\" not found.")
...
def move_to_folder(file_id, folder_id):
    ...
    try:
        ...
        log_file().info("Source Compiler.log", "Moved \"" + filename + "\" (ID: %s) to " % file_id + folder_name + " (ID: %s)" % parent_id)
        print("Moved \"" + filename + "\" (ID: %s) to " % file_id + folder_name + " (ID: %s)" % parent_id)
    except TypeError:
        log_file().error("Source Compiler.log", "Could not find file to move.")
        print("Could not find file to move.")    
    except googleapiclient.errors.HttpError:
        log_file().error("Source Compiler.log", "Invalid folder ID.")
...
def upload_excel(file_name):
    ...
    try:
        ...
        file = drive_service.files().create(body=file_metadata,
                                            media_body=media,
                                            fields='id').execute()
        log_file().info("Source Compiler.log", "Uploaded " + file_name + "(ID: %s) to Drive" % file.get('id'))
        print("Uploaded " + file_name + "(ID: %s) to Drive" % file.get('id'))
    except IOError:
        log_file().error("Source Compiler.log", "File not found.")
