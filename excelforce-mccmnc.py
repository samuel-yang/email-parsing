...
from decimal import *
from Google_Drive_Manipulation import *
...
class format():
    ...
    column_dictionary =({'Country': ['Country']},
                        ...
                        {'MCC': ['MCC','MCCMNC','Network code']},
                        {'MNC': ['MNC','MCCMNC','Network code']},
        ...
        for row in range(rownum):
            for y in range(colnum):
                ...
                # """MCC"""
                    ...
                    if sheet.cell(row,y).value == "MCCMNC" or "Network code":
                        for x in range(row+1, rownum):
                            value=sheet.cell(x,y).value
                            value=string[:3]
                ...
                # """MNC"""
                if sheet.cell(row,y).value in column_dictionary[4][column_list[4]]:
                    if sheet.cell(row,y).value == "MCCMNC" or "Network code":
                        for x in range(row+1, rownum):
                            value=sheet.cell(x,y).value
                            value=string[3:]
                ...
                # """MCCMNC"""
                if sheet.cell(row,y).value in column_dictionary[5][column_list[5]]:
                    mccmnc_absent=False
                    for x in range(row+1, rownum):
                        value = sheet.cell(x,y).value
                ...
                #Computing MCCMNC       
                if mccmnc_absent=True:
                    for x in range(row+1,rownum):
                        if "," not in sheet.cell(x,3):
                            value=sheet.cell(x,2).value + sheet.cell(x,3).value
                            sheet_wr.write(x-row,4,value)
                        else:
                            value=""
                            sheet_wr.write(x-row,4,value)
