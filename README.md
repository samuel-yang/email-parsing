...
How it works
E-mails received with file attachments that detail the various rates from aggregators.
Attachments are automatically pulled into Google Drive every hour, using a Drive add-on.
Files are downloaded locally, to be processed by Source_Compiler.py
File names are referenced against the inbox messages to determine the sender of each file.
Sender is referenced against a google sheet that specifies Source Name by e-mail.
...
Psuedo code
Downloads currency exchange workbook if non existent.  This workbook contains all rates for specified currencies from the initialization of this program to current day. 
Generates 3 root nodes, a premium and wholesale root with the data list in the same format, and an additional change root that tracks changes in pricing.
Downloads files using dl_folder(folder_id), which returns a list of all files downloaded.  
If no files downloaded, program completes and sleeps.
Since all files are renamed with the effective date appended into the name, a check is performed to rename these files back to their original if they are being re-processed.  
List of downloaded files is given to get_email_attachment_list(dl_list) which returns a list of lists of all necessary information.
Files are searched for in the attachments of all messages marked ‘NEW’ in the inbox.  A check is performed to see if there are no messages that match this requirement.  (Should only occur when re-running the program on already processed files) Completes the program and puts it to sleep.
...
Binary search tree (BST) is built using the most current (with respect to the oldest file being processed) rate sheet.  
List is removed one at a time from the returned list from step 3, file sources are compared to general_dictionary and special_dictionary to determine if the file is able to processed.  If file cannot be processed, a message is generated and logged and file is moved into a corresponding Not Processed folder.  
Each file’s effective date is checked with regards to the previously processed document.  If the effective date has changed, that rate sheet is written to, and uploaded to the drive.  The new date’s database is built on top of the new existing data, so that all data is preserved.
If the date of the newest file is behind the date the program is being processed, it builds to current day to update all records.
Currency exchange sheet is uploaded with new information. 
Sleeps for 30 minutes or 1800 seconds.
